app.controller('LeaderCtrl', ['$scope', '$http', 'filterFilter', function ($scope, $http, filterFilter) {
    $scope.tes = [];
    $scope.pers = [];
    $scope.totalItems = 0;
    $scope.currentPage = 1;
    ...
    $http({
        method: "GET",
        url: "http://127.0.0.1:8000/api/home-teacher"
    }).then(function (data) {
        // $scope.tes = data.data;
        data.data.forEach(function (item, i) {
            if (item.ID_Permission == 'Q005') {
                $scope.tes.push(item);
            }
        });
        $scope.totalItems = $scope.tes.length;
    });
    ...
    $scope.$watch('search', function (newVal, oldVal) {
        $scope.filtered = filterFilter($scope.tes, newVal);
