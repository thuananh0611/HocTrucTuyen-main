app.controller('parentstudentAdControl', ['$scope', '$http', 'filterFilter', function ($scope, $http, filterFilter) {
    $scope.parentStus = [];
    $scope.stus = [];
    $scope.totalItems = 0;
    $scope.currentPage = 1;
    ...
    $http({
        method: "GET",
        url: "http://127.0.0.1:8000/api/parentStudent"
    }).then(function (data) {
        $scope.parentStus = data.data;
        $scope.totalItems = $scope.parentStus.length;
    ...
    $scope.$watch('search', function (newVal, oldVal) {
        $scope.filtered = filterFilter($scope.parentStus, newVal);
    ...
    $scope.saveData = function () {
        if ($scope.id == 0) {
            ...
            $http({
                method: "PUT",
                url: "http://127.0.0.1:8000/api/parentStudent/" + $scope.id,
                data: $scope.parentStu,
                'content-Type': "application/json"
            }).then(function (res) {
    ...
    $scope.deleteClick = function (id) {
        ...
        if (hoi) {
            $http({
                method: "DELETE",
                url: "http://127.0.0.1:8000/api/parentStudent/" + id
    ...
    $scope.showModal = function (id) {
        $scope.id = id;
        if (id == 0) {
            $scope.modalTitle = "Thêm Phụ huynh học sinh mới";
            $scope.modalButton = "Thêm mới";
            if ($scope.parentStu) {
                $scope.parentStu.ID_Student = "";
                $scope.parentStu.Name_Dad = "";
                $scope.parentStu.Birth_Dad = "";
                $scope.parentStu.National_Dad = "";
                $scope.parentStu.Ethnic_Dad = "";
                $scope.parentStu.Religion_Dad = "";
                $scope.parentStu.Address_Dad = "";
                $scope.parentStu.Job_Dad = "";
                $scope.parentStu.Phone_Dad = "";
                $scope.parentStu.Name_Mom = "";
                $scope.parentStu.Birth_Mom = "";
                $scope.parentStu.National_Mom = "";
                $scope.parentStu.Ethnic_Mom = "";
                $scope.parentStu.Religion_Mom = "";
                $scope.parentStu.Address_Mom = "";
                $scope.parentStu.Job_Mom = "";
                $scope.parentStu.Phone_Mom = "";
            }
        } 
        else {
            $scope.modalTitle = "Cập nhật thông tin Phụ huynh học sinh";
